...
public class ObjLoader {
	...
	private Vector<Vertex>		mFinalVertices;
	...
	public void load(Context context, InputStream in) {
	    ...
	    while(itr.hasNext()) {
			...
			if (mTexCoords.size() > 0) {
		    	...
		    	uv1 = mTexCoords.get( f.getTexCoordsIdx()[0]);
				uv2 = mTexCoords.get( f.getTexCoordsIdx()[1]);
				uv3 = mTexCoords.get( f.getTexCoordsIdx()[2]);
			} else {
				// No texture coordinates defined.
				uv1 = new Vec2(0f, 0f);
				uv2 = new Vec2(0f, 1f);
				uv3 = new Vec2(1f, 1f);
			}
	    ...
	    //mIndices = new short[triangles.size() * 3];
	    mIndices		 = new Vector<Short>();
	    mFinalVertices	= new Vector<Vertex>();
	    //int idx = 0;
	    ...
	    while(TriangleItr.hasNext()) {
	    	...
	    	if (index == -1) {
	    		...
	    		mIndices.add((short)(mFinalVertices.size() - 1));
	    	} else {
